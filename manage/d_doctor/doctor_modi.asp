...
function GoToURL()
{
  ...
  var tURL= "doctor_modi.asp?hs_no="+document.un.hs_no.options[document.un.hs_no.selectedIndex].value;
...
function GoToURLA()
{
  ...
  var tURL1 = "doctor_modi.asp?yy="+document.un.yy.value;
...
<tr bgcolor="#FFFFFF"> 
    ...
    <tr bgcolor="#FFFFFF"> 
       ...
       <select name="ysclass" >
        <option value="首席大医生" > 首席大医生</option>
		   <option value="大医生" > 大医生</option>
			 ...
			 <option value="<%=rs("ysclass")%>" selected><%=rs("ysclass")%></option>
		  </select>
          </td>
    </tr>
	  <tr bgcolor="#FFFFFF"> 
      ...
      <td width="79%" class="main"> 
        <input type="text" name="dq" size="50" value="<%=rs("dq")%>">
  ...
  <option value="<%=rs("zljd")%>" selected> <%=rs("zljd")%></option>
    ...
    XMCG_GJ=RS("XMCG_GJ")
     ...
     %>
          <textarea name="XMCG_GJ" rows="10" cols="60"><%=XMCG_GJ%></textarea> 
          </td>
    </tr>
	 ...
	 <tr bgcolor="#FFFFFF"> 
      ...
      <td width="79%" class="main"> 
		<input type="hidden" value="<%=rs("zp")%>" name="yzp">
		 <input type="text" name="zp" size="16" value="<%=rs("zp")%>"><font color=red>输入名次就可以了，FTP上传，制作小的图片400X600的，可以为空</font>
       </td>
    </tr>
