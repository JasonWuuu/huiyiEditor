...
Function MHYZCH(DIS)
		MYINPUT = DIS
		'判断工作职称
				SQLLIB = "SELECT * FROM GUOLV WHERE ZD = 'MYZCH' ORDER BY NUM"
        RSLIB.OPEN SQLLIB,CONN,1,1
					LSTH = ""
					DO WHILE NOT RSLIB.EOF 
						IF RSLIB("QH") = "后" THEN
								POP = 0 
								POP = INSTR(MYINPUT,RSLIB("ZF"))
									IF POP >0 THEN
										'判断前字符是否为空
										IF RSLIB("QZF") = "" THEN
										LSTH = LSTH & RSLIB("ZF")
...
Function MYZC(DIS)
	MYINPUT = DIS
	SQLLIB = "SELECT * FROM GUOLV WHERE ZD = 'MYZC' ORDER BY NUM"
        RSLIB.OPEN SQLLIB,CONN,1,1
					LSTH = ""
					SFZD = 0
					DO WHILE NOT RSLIB.EOF AND SFZD <1
						IF RSLIB("QH") = "后" THEN
								POP = 0 
								POP = INSTR(MYINPUT,RSLIB("ZF"))
									IF POP >0 THEN
...
Function MHYXS(DIS)
		MYINPUT = DIS
		'判断工作职称
				SQLLIB = "SELECT * FROM GUOLV WHERE ZD = 'MYXS' ORDER BY NUM"
        RSLIB.OPEN SQLLIB,CONN,1,1
					LSTH = ""
					DO WHILE NOT RSLIB.EOF 
						IF RSLIB("QH") = "后" THEN
...
Function MHYMYEDU_OTHERS(DIS)
        ...
        RSLIB.OPEN SQLLIB,CONN,1,1
					...
					DO WHILE NOT RSLIB.EOF 
						...
						POP = INSTR(MYINPUT,RSLIB("ZF"))
							IF POP >0 THEN
								IF RSLIB("QZF") = "" THEN
									'如果为空，就判断一句话的标点符号
										MYINPUT = MID(MYINPUT,POP)
										POP1 = 0
										POP1 = INSTR(MYINPUT,"、")
											IF POP1 >0 THEN
											LSTH = LSTH & MID(MYINPUT,POP1+1)
											ELSE
												POP1 = 0
												POP1 = INSTR(MYINPUT,"；")
												IF POP1 >0 THEN
					...
					LOOP
				RSLIB.CLOSE 
	MHYMYEDU_OTHERS = LSTH
End Function

Function MHYRWGS(DIS)
End Function
...
'处理字符
		...
		MYINPUT = Request("myinput")
			POP = 0
			POP = INSTR(MYINPUT,"客座教授")
				IF POP >0 THEN
				MYINPUT = MID(MYINPUT,1,POP+4)
				POP1 = 0
					POP1 = INSTR(-1,MYINPUT,"、")
					IF POP1 >0 THEN
					MYZC = MID(MYINPUT,POP1+1)
					ELSE
					POP1 = 0 
						POP1 = INSTR(-1,MYINPUT,"；")
						IF POP1 >0 THEN
				...
				END IF
			POP = 0 
			POP = INSTR(MYINPUT,"副教授")
				IF POP >0 THEN
						
					ELSE
					POP = 0
						POP = INSTR(MYINPUT,"教授")
						IF POP >0 THEN
							MYINPUT = MID(MYINPUT,1,POP+3)
						POP1 = 0
						POP1 = INSTR(-1,MYINPUT,"、")
							IF POP1>0 THEN
							MYZC = MID(MYINPUT,POP1+1) & "、" & MYZC
							ELSE
								POP1 = 0
								POP1 = INSTR(-1,MYINPUT,"；")
									IF POP1 >0 THEN
			...
			POP = 0 
			POP = INSTR(MYINPUT,"副研究员")
				IF POP >0 THEN
						
					ELSE
					POP = 0
						POP = INSTR(MYINPUT,"研究员")
						IF POP >0 THEN
							MYINPUT = MID(MYINPUT,1,POP+3)
						POP1 = 0
						POP1 = INSTR(-1,MYINPUT,"、")
...
function GoToURLB()
{
  ...
  var tURL2= "&class_no="+document.un.class_no.options[document.un.class_no.selectedIndex].value;
  var tURL1 = "&yy="+document.un.yy.value;
  document.location=tURL+tURL1+tURL2;
}
</script>
<script language="javascript">
function GoToURLA()
   ...
   <option value="诊治" > 诊治</option>
    ...
    <tr bgcolor="#FFFFFF"> 
	 ...
	 <tr bgcolor="#FFFFFF"> 
      ...
      <td width="79%" class="main"> 
		 <input type="text" name="qg_pm" size="46"  value="0">
       </td>
    </tr>
	 ...
	 <tr bgcolor="#FFFFFF"> 
      ...
      <td width="79%" class="main"> 
		 <input type="text" name="bj_qg_pm" size="46"  value="0">
       </td>
    </tr>
	 ...
	 <tr bgcolor="#FFFFFF"> 
      ...
      <td width="79%" class="main"> 
		 <input type="text" name="jci_qg_pm" size="46"  value="0">
       </td>
    </tr>
    <tr bgcolor="#FFFFFF"> 
      <td colspan="2"> 
