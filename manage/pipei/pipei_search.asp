...
'操作：本脚本任何人操作，原则上是自己删除自己的记录
	...
	END IF
	IF REQUEST("ADD") <> "" THEN
	Response.Redirect ("pipei_add.asp")
	END IF
	%> 
<%
DIM LK(20)
...
IF REQUEST("DEL") <> "" THEN
	...
	FOR J = 0 TO I
	  ...
	  SQL = "SELECT * FROM pipei WHERE id = " & LK(J) & " "
	  	RS.Open SQL,CONN,adOpenKeyset ,adLockReadOnly
			IF RS.RecordCount >0 THEN
				SQL = "DELETE FROM pipei WHERE id = " & LK(J) & ""
				CONN.Execute(SQL)
			END IF
		RS.Close 
...
'查询
	...
	IF REQUEST("mc") <> "" THEN
	MYB = " AND mc like  '%" & REQUEST("mc") & "%' "
	END IF

	SQL = "SELECT  * FROM pipei WHERE mc <> ''  " & MYA & MYB  & " ORDER BY id DESC"
	RS.Open SQL,CONN,adOpenKeyset ,adLockReadOnly
	IF RS.RecordCount >0 THEN
	RS.MoveLast 
			RECO=RS.RecordCount 
...
<form method="post" action="pipei_search.asp" name="un">
  ...
  <table width="898" border="0" bordercolordark="#99CCFF" bordercolorlight="#99CCFF" cellspacing="1" align="center" cellpadding="6" bgcolor="#000000" class="main">
   ...
   <td width="11%"> 
	   ...
	   <td width="11%"> 
        <div align="center"><font color="#0000FF">内容</font></div>
      </td>
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td width="5%" height="34"> <a href="ghpt_modi.asp?id=<%=RS("id")%>" target="_blank">Go!</a> 
       ...
       <td width="21%" height="34"> 
        <div align="center"><%=RS("mc")%></div>
      </td>
	
     <td width="21%" height="34"> 
        <div align="center"><%=RS("url")%></div>
      </td>
		
	   <td width="21%" height="34"> 
        <div align="center"><%=RS("note")%></div>
      </td>
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td width="12%"> 
        <div align="center"> <font color="#000000"><%
                ...
                %><a href="pipei_search.asp?url=<%=request("url")%>&mc=<%=request("mc")%>&PageNo=<%=PageNo-1%>&search=all">上一页 
          </a> <%
                ELSE
                %> 上一页 <%
                END IF
      ...
      <td width="12%"> 
        <div align="center"> <font color="#000000"><%
                ...
                %><a href="pipei_search.asp?url=<%=request("url")%>&mc=<%=request("mc")%>&PageNo=<%=PageNo+1%>&search=all"> 
          下一页</a> <%
                ELSE
                %> 下一页 <%
                END IF
