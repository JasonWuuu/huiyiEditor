...
<%
   ...
   if Request("save") <> "" Then
					...
					NO = NO
					END IF
					IF INT(NO) <1000 AND INT(NO) >=100 THEN
					NO = "0" & NO
					END IF
					IF INT(NO) <100 AND INT(NO) >=10 THEN
					NO = "00" & NO
					END IF
					IF INT(NO) <10 THEN
					NO = "000" & NO
					...
					'sfz
					if len(Request("sfz"))<> 18 then
						Response.Write("身份证号码不对")
						Response.end 
					End if
					...
					'增加记录
	  				...
	  				SQL = "SELECT top 1 * FROM SHANGFANG"
...
<form id="form1" name="un" method="post" action="save.asp">
  <table border="1" cellspacing="0" cellpadding="2" bordercolorlight="#FFFFFF" class="main" width="907">
    ...
    <tr> 
      <td width="92"> 
        <div align="right"><input type="submit" name="add" value="继续增加"></div>
      </td>
      <td colspan="3"> 
        <div align="center"> <input type="submit" name="home" value="返回主页"> </div>
      </td>
      <td width="123"><input type="submit" name="search" value="进入查询"></td>
      <td colspan="3">&nbsp;</td>
    </tr>
  </table>
