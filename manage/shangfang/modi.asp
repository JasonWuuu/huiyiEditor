	...
	SQL = "SELECT TOP 1 * FROM SHANGFANG WHERE ID = " & REQUEST("ID") & " AND PERSON = '" & Request.Cookies("user_name") & "'"
		RS.Open SQL,CONN,1,1
			IF RS.RecordCount >0 THEN
				ELSE
				Response.Write("数据库出错误！或非本人输入的记录")
				Response.end 
			END IF
...
<table width="980" border="0" cellspacing="0" cellpadding="5" class="main">
  <tr>
    <td class="wz">
      <div align="center"><b>来访人员登记表（修改）</b></div>
    </td>
  </tr>
</table>
...
<input type="hidden" name="id" value="<%=Request("id")%>">
  <table border="1" cellspacing="0" cellpadding="2" bordercolorlight="#FFFFFF" class="main" width="900">
    ...
    <tr> 
      ...
      <td>
        <div align="right">初访时间 格式：YYYY-MM-DD </div>
      </td>
      <td> 
        <div align="left">
      ...
      <td colspan="3"> 
        <div align="center"> 
          ...
          <input type="reset" name="Reset" value="重新输入">
			  <input type="button" name="home" value="返    回" onClick=javascript:window.close();>
        </div>
      </td>
      <td>&nbsp;</td>
      <td colspan="3">&nbsp;</td>
