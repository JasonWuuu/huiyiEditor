...
IF REQUEST("DEL") <> "" THEN
	...
	FOR J = 0 TO I
	  ...
	  SQL = "SELECT * FROM GUOLV WHERE ID = " & LK(J) & " "
	  	RS.Open SQL,CONN,adOpenKeyset ,adLockReadOnly
			IF RS.RecordCount >0 THEN

			SQL = "DELETE FROM GUOLV WHERE ID = " & LK(J) & ""
			CONN.Execute (SQL)
...
'查询
	...
	IF REQUEST("HZF") <> "" THEN
	MYB = " AND HZF LIKE  '%" & REQUEST("HZF") & "%' "
	END IF
	
	SQL = "SELECT * FROM GUOLV WHERE ZD <> ''  " & MYA & MYB  & " ORDER BY zd"
	RS.Open SQL,CONN,adOpenKeyset ,adLockReadOnly
	IF RS.RecordCount >0 THEN
	RS.MoveLast 
			RECO=RS.RecordCount 
...
<form method="post" action="a_corre_search.asp" name="un">
  <input type=hidden name="zd" value="<%=REQUEST("zd")%>">
  <input type=hidden name="hzf" value="<%=REQUEST("hzf")%>">
  ...
  <table width="898" border="0" bordercolordark="#99CCFF" bordercolorlight="#99CCFF" cellspacing="1" align="center" cellpadding="6" bgcolor="#000000" class="main">
    <tr bgcolor="#CCCCCC"> 
      ...
      <td width="9%"><font color="#0000FF">字段名称</font></td>
      <td width="11%"> 
        <div align="center"><font color="#0000FF">前字符</font></div>
      </td>
  		<td width="11%"> 
        <div align="center"><font color="#0000FF">后字符</font></div>
      </td>
	    </tr>
    <%
		'判断显示的总页数
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td width="5%" height="34"> <a href="a_corre_modi.asp?id=<%=RS("id")%>" target="_blank">Go!</a> 
        &nbsp; </td>
      <td width="5%" height="34"><%=RS("zdmc")%> </td>
	    <td width="9%" height="34"><%=RS("qzf")%> 
 	  <td width="9%" height="34"><%=RS("hzf")%> 
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td width="12%"> 
        <div align="center"> <font color="#000000"><%
                ...
                %><a href="a_corre_search.asp?zd=<%=request("zd")%>&hzf=<%=request("hzf")%>&PageNo=<%=PageNo-1%>&search=all">上一页 
          </a> <%
                ELSE
                %> 上一页 <%
                END IF
      ...
      <td width="12%"> 
        <div align="center"> <font color="#000000"><%
                ...
                %> <a href="a_corre_search.asp?zd=<%=request("zd")%>&hzf=<%=request("hzf")%>&PageNo=<%=PageNo+1%>&search=all">下一页 
         </a> <%
                ELSE
                %> 下一页 <%
                END IF
