...
IF REQUEST("DEL") <> "" THEN
	...
	FOR J = 0 TO I
	  ...
	  SQL = "SELECT * FROM D_ghpt WHERE id = " & LK(J) & " "
	  	RS.Open SQL,CONN,adOpenKeyset ,adLockReadOnly
			IF RS.RecordCount >0 THEN
				SQL = "DELETE FROM D_ghpt WHERE id = " & LK(J) & ""
				CONN.Execute(SQL)
			END IF
		RS.Close 
...
'查询
	IF REQUEST("CLASS") <> "" THEN
		MYA = " AND CLASS LIKE '%" & REQUEST("CLASS") & "%' "
		ELSE
		MYA = "  "
	...
	MYB = " AND NAME like  '%" & REQUEST("NAME") & "%' "
	END IF

	SQL = "SELECT  * FROM D_GHPT WHERE NAME <> ''  " & MYA & MYB  & " ORDER BY NUM DESC"
	RS.Open SQL,CONN,adOpenKeyset ,adLockReadOnly
	IF RS.RecordCount >0 THEN
	RS.MoveLast 
			RECO=RS.RecordCount 
...
<form method="post" action="ghpt_search.asp" name="un">
  ...
  <table width="898" border="0" bordercolordark="#99CCFF" bordercolorlight="#99CCFF" cellspacing="1" align="center" cellpadding="6" bgcolor="#000000" class="main">
    <tr bgcolor="#CCCCCC"> 
      ...
      <td width="11%"> 
        <div align="center"><font color="#0000FF">名称</font></div>
      </td>
      <td width="11%"> 
        <div align="center"><font color="#0000FF">类别</font></div>
      </td>
   ...
   <td width="11%"> 
        <div align="center"><font color="#0000FF">URL</font></div>
		 ...
		 <td width="11%"> 
        <div align="center"><font color="#0000FF">IMG</font></div>
      </td>
	   <td width="11%"> 
        <div align="center"><font color="#0000FF">显示顺序</font></div>
      </td>
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td width="5%" height="34"> <a href="ghpt_modi.asp?id=<%=RS("id")%>" target="_blank">Go!</a> 
        &nbsp; </td>
       ...
       <td width="21%" height="34"> 
		...
		<td width="21%" height="34"> 
        <div align="center"><%=RS("class")%></div>
      </td>
     <td width="21%" height="34"> 
        <div align="center"><a href="<%=RS("url")%>" target="_blank"><%=RS("url")%></a></div>
      </td>
		 <td width="11%" height="34"> 
       <div align="center"><img src="http://img.dakayi.cc/pic/<%=RS("img")%>" width="50px;"></div>
      </td>
	   <td width="21%" height="34"> 
        <div align="center"><%=RS("num")%></div>
      </td>
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td width="12%"> 
        <div align="center"> <font color="#000000"><%
                ...
                %><a href="ghpt_search.asp?class=<%=request("class")%>&name=<%=request("name")%>&PageNo=<%=PageNo-1%>&search=all">上一页 
          </a> <%
                ELSE
                %> 上一页 <%
                END IF
      ...
      <td width="12%"> 
        <div align="center"> <font color="#000000"><%
                ...
                %><a href="ghpt_search.asp?class=<%=request("class")%>&name=<%=request("name")%>&PageNo=<%=PageNo+1%>&search=all"> 
          下一页</a> <%
                ELSE
                %> 下一页 <%
                END IF
