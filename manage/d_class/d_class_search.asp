...
IF REQUEST("DEL") <> "" THEN
	...
	FOR J = 0 TO I
	  ...
	  SQL = "SELECT * FROM D_CLASS WHERE CLASS_NO = '" & LK(J) & "' "
	  	RS.Open SQL,CONN,adOpenKeyset ,adLockReadOnly
			IF RS.RecordCount >0 THEN

			SQL = "DELETE FROM  D_CLASS WHERE CLASS_NO = '" & LK(J) & "'"
			CONN.Execute (SQL)
...
'查询
	...
	SQL = "SELECT * FROM D_CLASS WHERE CLASS_NO <> ''  " & MYA & MYB  & " ORDER BY NUM"
	RS.Open SQL,CONN,adOpenKeyset ,adLockReadOnly
	IF RS.RecordCount >0 THEN
	RS.MoveLast 
			RECO=RS.RecordCount 
...
<form method="post" action="d_class_search.asp" name="un">
  ...
  <table width="898" border="0" bordercolordark="#99CCFF" bordercolorlight="#99CCFF" cellspacing="1" align="center" cellpadding="6" bgcolor="#000000" class="main">
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td width="5%" height="34"> <a href="d_class_modi.asp?class_no=<%=RS("class_no")%>" target="_blank">Go!</a> 
        &nbsp; </td>
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td width="12%"> 
        <div align="center"> <font color="#000000"><%
                ...
                %><a href="d_class_search.asp?hs_no=<%=request("hs_no")%>&class_name=<%=request("class_name")%>&PageNo=<%=PageNo-1%>&search=all">上一页 
          </a> <%
                ELSE
                %> 上一页 <%
                END IF
      ...
      <td width="12%"> 
        <div align="center"> <font color="#000000"><%
                ...
                %> <a href="d_class_search.asp?hs_no=<%=request("hs_no")%>&class_name=<%=request("class_name")%>&PageNo=<%=PageNo+1%>&search=all">下一页 
         </a> <%
                ELSE
                %> 下一页 <%
                END IF
